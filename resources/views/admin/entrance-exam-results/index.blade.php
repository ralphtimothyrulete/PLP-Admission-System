...
@section('body')
<h3 class="text-3xl font-bold text-black mb-5">Entrance Exam Results</h3>
<a href="{{ route('entrance-exam-results.create') }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">Create New Result</a>

<div class="flex flex-col mt-8">
    <div class="mb-4 flex items-center">
        <form action="{{ route('entrance-exam-results.upload') }}" method="POST" enctype="multipart/form-data" class="flex items-center">
            @csrf
    ...
    @endif
    <div class="mb-4">
        <form action="{{ route('entrance-exam-results.index') }}" method="GET" class="flex items-center">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name or result" class="w-60 p-2 border rounded ml-auto">
                ...
                <tbody class="bg-white">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                            <div class="text-sm leading-5 font-bold text-gray-900">{{ $result->id }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                            <div class="text-sm leading-5 font-medium text-gray-900">{{ $result->student_id }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                            <div class="text-sm leading-5 text-gray-900">{{ $result->name }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                            <div class="px-1 inline-flex text-xs leading-5 font-bold rounded-full bg-green-100 text-green-800">{{ $result->result }}</div>
                        </td>
                        <td class="px-10 py-4 whitespace-no-wrap text-left border-b border-gray-200 text-sm leading-5 font-medium">
                            <button onclick="confirmDelete('{{ $result->id }}')" class="text-red-600 hover:text-red-900 ml-11">Delete</button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Delete Result</h3>
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">Are you sure you want to delete this result? This action cannot be undone.</p>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    function confirmDelete(id) {
        document.getElementById('deleteForm').action = '/entrance-exam-results/' + id;
